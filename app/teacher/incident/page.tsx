...
const SEVERITY_BADGE: Record<IncidentSeverity, string> = {
  ...
  MAJOR: "bg-amber-50 text-amber-600 border-amber-300",
  MINOR: "bg-blue-50 text-blue-600 border-blue-300",
};
...
function IncidentCard({ incident, onClose }: { incident: Incident; onClose: () => void }) {
  ...
  return (
    <Card className={`border ${incident.종결여부 ? "border-gray-200 opacity-60" : SEVERITY_BORDER[incident.심각도]}`}>
      <CardContent className="pt-4 pb-3 space-y-2">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            <Badge variant="outline" className={SEVERITY_BADGE[incident.심각도]}>
            ...
            {incident.종결여부 && (
              <Badge variant="outline" className="border-green-300 text-green-600 bg-green-50">종결</Badge>
            )}
          </div>
          <span className="text-xs text-gray-400 shrink-0">{timeStr}</span>
        </div>

        <p className="text-sm text-gray-800">{incident.내용}</p>
        {incident.조치 && <p className="text-xs text-gray-500">조치: {incident.조치}</p>}
        {incident.관련학생.length > 0 && (
          <p className="text-xs text-gray-400">관련: {incident.관련학생.map((r) => r.split("/").pop()).join(", ")}</p>
        )}

        {!incident.종결여부 && (
          <Button size="sm" variant="outline"
            className="border-green-300 text-green-600 hover:bg-green-50 mt-1"
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        <div className="space-y-2">
          ...
          {open.length === 0 ? (
            ...
            open.map((i) => <IncidentCard key={i.id} incident={i} onClose={loadData} />)
          )}
        </div>
        ...
        {closed.length > 0 && (
          <div className="space-y-2">
            ...
            {showClosed && closed.map((i) => <IncidentCard key={i.id} incident={i} onClose={loadData} />)}
          </div>
        )}
      </main>
    </div>
